...
let get_table table keys =
  ...
  | TTable table -> table
  | _ ->
      failwith "Wrong key type %s: expexted Table in %s"
        (String.concat "." keys)
        (Toml.Printer.string_of_table table)
...
let get_int table keys =
  match get table keys with TInt i -> i | _ -> raise Not_found

let get_int_default table keys default =
  match get_int table keys with exception Not_found -> default | i -> i
...
let put_bool_option key bo table =
  match bo with None -> table | Some b -> put_bool key b table

let put_option keys so table =
  match so with None -> table | Some v -> put_bool keys v table

let put_string_option keys so table =
  match so with None -> table | Some s -> put_string keys s table
...
let put_encoding_option encoding key vo table =
  match vo with None -> table | Some v -> put_encoding encoding key v table
...
module CONST = struct
  ...
  let string_option = function None -> None | Some s -> string s

  let string_list l = Some (TArray (NodeString l))

  let string_list_option = function None -> None | Some l -> string_list l

  let encoding enc v = Some (enc.to_toml v)

  let encoding_option enc = function None -> None | Some b -> encoding enc b

  let bool b = Some (TBool b)

  let bool_option = function None -> None | Some b -> bool b
